...
while getopts ":ht:c:i:v:q:y:b:k:r:s:w:-:" opt; do
 case $opt in
  -)
     case "${OPTARG}" in
       dashboardname)
...
echo "e-upgrader SAVE_CLONE=$SAVE_CLONE"

echo "e-upgrader USE_SAVED_CLONE=$USE_SAVED_CLONE"

echo "e-upgrader CLONE_FILE_NAME=$CLONE_FILE_NAME"
