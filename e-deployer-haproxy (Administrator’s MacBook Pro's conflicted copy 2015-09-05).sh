...
while getopts ":ht:p:d:u:w:c:i:v:q:l:y:b:k:-:" opt; do
 case $opt in
  -)
     case "${OPTARG}" in
       dashboardname)
...
svn export svn+ssh://svn.naehas.com/home/svn/repository/base-dashboard/trunk/setup-haproxy setup-haproxy --force
...
# Shutdown
#if [ -d "$DIR_NAME" ]; then
	echo "Calling ./shutdown again just in case the previous deployment failed but has already attempted a prior startup. If it errors out, please ignore."
	cd bin
	./shutdown.sh
	cd ..
#fi
