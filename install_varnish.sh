...
echo -n "install automake autoconf libtool ncurses-devel libxslt groff pcre-devel pkgconfig gcc please wait ...... "
eval "${YUM} install -y automake autoconf libtool ncurses-devel libxslt groff pcre-devel pkgconfig gcc gcc-c++ make >${log_file} 2>&1" || yum_install='fail'
...
install_pcre () {
	...
	#run_cmds './configure --prefix=/usr/local/pcre/' 'make' 'make install'
...
install_varnish () {
	...
	run_cmds './configure --enable-dependency-tracking  --enable-debugging-symbols  --enable-developer-warnings' 'make' 'make install'
	...
	test -e /etc/sysconfig/varnish && sed -i '/-S ${VARNISH_SECRET_FILE}/d' /etc/sysconfig/varnish
	cd ..
}
