...
nofdays=45
sourcedir='/data4/db6/'
logfile='/tmp/tablescompressed-db6-dmiplatform-LM.log'
tmpdir='/tmp'
#Below are the most important parameters
DB='dmiplatform_staging'
MYSQL="mysql --socket=/var/lib/mysql/db6.sock $DB -A  -BCe"

# Split the master query into multiple small tables
$MYSQL "DROP TABLE if exists old_staging_tables; CREATE TABLE old_staging_tables SELECT distinct tablename, LAST_MODIFIED_DATE from N_DATA_LISTS where type in ('DATA_FILE','TMP','STAGING') AND LAST_MODIFIED_DATE < date_sub(now(),INTERVAL $nofdays DAY) AND tablename like 'LM%';"
...
$MYSQL "SELECT a.tablename from old_staging_tables a, is_tablelist b WHERE a.tablename=b.table_name"> $tmpdir/tablelist-LM-$DB.txt
...
done < $tmpdir/tablelist-LM-$DB.txt
