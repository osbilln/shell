...
logfile1="/tmp/all-db-mysqlimport"
tmpdir='/tmp'
...
while read db
do
        echo "Running mysqlimport for $db ...before the shrink" >> $logfile1
	chmod u+x $work_dir/mysqlimport_${db}.sh
	$work_dir/mysqlimport_${db}.sh
	echo "Completed running mysqlimport for $db" >> $logfile1
