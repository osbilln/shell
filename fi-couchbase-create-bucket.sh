...
/opt/couchbase/bin/couchbase-cli bucket-list -c ${cb_host}:${cb_port} -u $cb_user -p $cb_pass | egrep "^[A-Za-z]+" | while read bucket; do
  echo "Deleting bucket [${bucket}] ..."
  /opt/couchbase/bin/couchbase-cli bucket-delete -c ${cb_host}:${cb_port} -u $cb_user -p $cb_pass --bucket=${bucket}
done


# make sure we have right quota for cluster
/opt/couchbase/bin/couchbase-cli cluster-init -c ${cb_host}:${cb_port} -u $cb_user -p $cb_pass --cluster-init-ramsize=${fi_quota}
...
echo $fi_buckets | tr ',' '\n' | while read bucket_info; do
  ...
  /opt/couchbase/bin/couchbase-cli bucket-create -c ${cb_host}:${cb_port} -u $cb_user -p $cb_pass \
    --bucket=${bucket_name} \
