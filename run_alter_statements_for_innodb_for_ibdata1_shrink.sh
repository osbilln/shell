...
logfile1="/tmp/all-db-alter-table-innodb.log"
tmpdir='/tmp'
...
#	$MYSQL < $work_dir/alter_table_myisam_${db}.sql
	echo "Runnin alter statements for $db ...after the shrink (to InnoDB back)" >> $logfile1
	$MYSQL < $work_dir/alter_table_innodb_${db}.sql
	echo "Completed running alter statements for InnoDB" >> $logfile1
