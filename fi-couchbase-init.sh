...
$cb_bin/couchbase-cli bucket-list -c ${cb_host}:${cb_port} -u $cb_user -p $cb_pass | egrep "^[A-Za-z]+" | while read bucket; do
  echo "Deleting bucket [${bucket}] ..."
$cb_bin/couchbase-cli bucket-delete -c ${cb_host}:${cb_port} -u $cb_user -p $cb_pass --bucket=${bucket}
done


# make sure we have right quota for cluster
$cb_bin/couchbase-cli cluster-init -c ${cb_host}:${cb_port} -u $cb_user -p $cb_pass --cluster-init-ramsize=${fi_quota}
